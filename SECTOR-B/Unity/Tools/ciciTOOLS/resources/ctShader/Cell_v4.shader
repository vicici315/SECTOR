...
sampler2D _texHair;
	...
	// 	float3x3 mT2W;
	// 	mT2W[0] = normalize( i.m_vT2W_0 );
	// 	mT2W[1] = normalize( i.m_vT2W_1 );
	// 	mT2W[2] = normalize( i.m_vT2W_2 );
...
// half3 NoL = saturate(dot(worldNormal ,tangentLightDir));
            ...
            OffsetViewDir.z += sin(_RampOSz * -_Time*5);
            fixed NdotV = dot(packedNormal, OffsetViewDir);
            float2 BRDuv = float2(NdotV * _RampValueA, diff);
            BRDuv.x = sin(_Time*2 + (BRDuv.x + BRDuv.y));
            BRDuv.y = cos(_Time*3 + (BRDuv.x + BRDuv.y));
            fixed3 BRDF = tex2D(_RampTex, BRDuv.xy + _RampOSz*0.4).rgb;

	float  NoL = dot(tangentNormal, _WorldSpaceLightPos0);
	float  NoL_s = saturate( NoL );
	NoL_s = g_Tex2D_Mask( _texHair, float2( NoL_s, 0.5f ) ).r;
	...
	float mask = step(0.01, dot(hairTex, 1));
                ...
                col += (NoL_s * hairSpecular * _SpeBrightness * mask * 0.1);
                ...
                return fixed4 (col*BRDFcolor*_RampColor * outline, 1.0);
