...
Shader "VIC/Vic_NextGenCharacter_v3"
{
    Properties
    {
        ...
        _ReceiveSoftHard ("Receive Soft/Hard", Range(0,1)) = 1
        _ReceiveColor ("Receive Shadow Color", Color) = (0,0,0,0)
    ...
    SubShader
    {
		...
		Pass //描边Pass在最上层，因此后写
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed fresnel = (_FresnelS)*(pow(1.0-saturate(dot(worldViewDir, worldNormal)), _FresnelP));
