...
#define FX_USE_HDR					1
#define FX_USE_ENCODEHDRTEXTURE		1
...
#define FX_PBR_USE_LM_POINTATTEN	1
...
//#define FX_PBR_USE_GLOBALBRIGHTNESS
	...
	#define g_Tex2D_ColorAlpha( tex, uv )		float4( g_Tex2D_Color( tex, uv ).rgb, g_Tex2D_Mask( tex##_a, uv ).r )
	#define g_Tex2D_UI( tex, uv )				float4( g_Tex2D_Mask( tex, uv ).rgb, g_Tex2D_Mask( tex##_a, uv ).r )
#else
	#define g_Tex2D_ColorAlpha( tex, uv )		g_Tex2D_Color( tex, uv )
	#define g_Tex2D_UI( tex, uv )				g_Tex2D_Mask( tex, uv )
#endif
...
inline float4 g_TexCube_HDRColor( samplerCUBE _tex, float3 _uv )
{
	float4 vColor = texCUBE( _tex, _uv );
#if FX_USE_ENCODEHDRTEXTURE
	vColor.rgb = g_DecodeHDR( vColor );
#endif
#if FX_USE_GAMMATEXTURE
	return float4( pow( vColor.rgb, 2.2f ), vColor.a );
#else
...
inline float4 g_TexCubeLod_HDRColor( samplerCUBE _tex, float4 _uv )
{
	float4 vColor = texCUBElod( _tex, _uv );
#if FX_USE_ENCODEHDRTEXTURE
	vColor.rgb = g_DecodeHDR( vColor );
#endif
#if FX_USE_GAMMATEXTURE
...
#ifdef FX_PBR_USE_GLOBALBRIGHTNESS
	float g_fGlobalBrightness;
#endif

inline fixed3 g_OutputColor( fixed3 _vInput )
{
#ifdef FX_PBR_USE_GLOBALBRIGHTNESS
	_vInput *= g_fGlobalBrightness;
#endif
...
inline half3 GetImageBasedReflectionLighting( half Roughness, float3 R, samplerCUBE texIBL, half CubemapMaxMip, half3 SkyLightColor )
{
	...
	half4 SpecularIBLSample = g_TexCubeLod_HDRColor( texIBL, float4( R, AbsoluteSpecularMip ) );
	half3 SpecularIBL = SpecularIBLSample.rgb * SkyLightColor;
