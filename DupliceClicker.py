...
if conf.has_option('set_DS', 'HOST'):
    ...
    conf.set('set_DS', 'HOST', 'localhost')
    s_host = 'localhost'
...
#菜单：文件
        ...
        id_moveId = wx.NewIdRef()  # 分配自定义ID
        self.smenu_filemove = wx.MenuItem(self.file_menu_2, id_moveId, "移动文件")
        image_stream = io.BytesIO(base64.b64decode(icondata.icons_data['icon_save']))
        image = wx.Image(image_stream, wx.BITMAP_TYPE_PNG)
        self.smenu_filemove.SetBitmap(wx.Bitmap(image))
        self.file_menu_2.Append(self.smenu_filemove)
        ...
        id_delemptyfol = wx.NewIdRef()  # 分配自定义ID
        self.smenu_delemptyfol = wx.MenuItem(self.file_menu_2, id_delemptyfol, "清除空目录")
        image_stream = io.BytesIO(base64.b64decode(icondata.icons_data['icon_del']))
        image = wx.Bitmap(wx.Image(image_stream, wx.BITMAP_TYPE_PNG))
        self.smenu_delemptyfol.SetBitmap(image)
        self.file_menu_2.Append(self.smenu_delemptyfol)
        ...
        self.Bind(wx.EVT_MENU, self.Move_list, id=id_moveId)
        self.Bind(wx.EVT_MENU, self.open_TiQu, id=id_openTiQu)
        self.Bind(wx.EVT_MENU, self.fn_delemptyfol, id=id_delemptyfol)
...
# 菜单：帮助
        ...
        self.menu_update = wx.MenuItem(self.help_menu, id_update, "检查更新")
        ...
        self.menu_Rupdate = wx.MenuItem(self.help_menu, id_Rupdate, "手动更新")
...
# Button获取复制文件添加数据库按钮：copyfile_btn
        ...
        copyfile_btn.SetToolTip('筛选未拷贝文件（右键双击刷新当前数据库内容）')
...
# Button删除已上传文件按钮：self.deloldfile_btn
        self.deloldfile_btn = wx.Button(self.panel, wx.ID_ANY, size=(38, 28))
        image_stream = io.BytesIO(base64.b64decode(icondata.icons_data['deloldfile']))
        image = wx.Image(image_stream, wx.BITMAP_TYPE_PNG)
        self.deloldfile_btn.SetBitmap(wx.Bitmap(image))
        self.deloldfile_btn.SetToolTip('删除已存在数据库文件')
        self.deloldfile_btn.Bind(wx.EVT_BUTTON, self.del_oldfile)
        self.deloldfile_btn.Hide()
        self.h_sizer.Add(self.deloldfile_btn, flag=wx.ALL, border=2)
#CheckBox遍历文件选项：search_fol
        if conf.has_option('set_DS', 'searchFol_chk'):
            svchk = eval(conf.get('set_DS', 'searchFol_chk'))
        else:
            svchk = False
        self.search_fol_chk = wx.CheckBox(self.panel)
        self.search_fol_chk.SetToolTip('勾选对当前路径进行数据预处理')
        self.search_fol_chk.SetValue(svchk)
        # self.search_fol_chk.SetValue(True)
...
# CheckBox子文件夹选项：sub_chk
        if conf.has_option('set_DS', 'subFol_chk'):
            subval = eval(conf.get('set_DS', 'subFol_chk'))
        else:
            subval = True
        self.sub_chk = wx.CheckBox(self.panel)
        self.sub_chk.SetToolTip('勾选时查重处理子文件夹，否则只处理根目录文件')
...
#ComboBox文件、文件夹：comb_types
        ...
        self.comb_tiqu.SetMaxSize((448, -1))
        try:
...
#ComboBox搜索内容历史列表：combox_S
        ...
        self.combox_S.SetMaxSize((220,-1))
...
#Button按钮：button
        ...
        # 创建 wx.Button，设置图标和标签
        image_stream = io.BytesIO(base64.b64decode(icondata.icons_data['icon_ds']))
        sicon = wx.Bitmap(wx.Image(image_stream, wx.BITMAP_TYPE_ICO))
        button = wx.Button(self.panel, wx.ID_ANY, label="搜索", size=(80, 28))
        button.SetBitmap(sicon)
...
#BoxSizer过滤文本和相关按钮的平行布局：ext_sizer
        ...
        self.rext_sizer = wx.BoxSizer(wx.HORIZONTAL)
    # 侧边按垂直布局
        self.btn_sizer = wx.BoxSizer(wx.VERTICAL)
    # 清除文件类型排除
        self.ext_clear_btn = wx.Button(self.panel, id=wx.ID_ANY, label='清', size=(30, 28))
        self.ext_clear_btn.SetBackgroundColour(wx.Colour(210,250,250))
        self.ext_clear_btn.Bind(wx.EVT_BUTTON,self.clear_allext)
        self.ext_clear_btn.SetToolTip('清除文件类型排除')
        self.btn_sizer.Add(self.ext_clear_btn,0,wx.TOP,0)
        self.ext_clear_btn.Hide()
    # 获取所有文件类型
    ...
    # TextCtrl检索路径文件过滤：ext_filter
        ...
        self.ext_filter.SetToolTip('输入排除类型后缀名，用;(分号)隔开')
    ...
    # 侧边按垂直布局
        self.rbtn_sizer = wx.BoxSizer(wx.VERTICAL)
    # 清除文件类型排除
        self.rext_clear_btn = wx.Button(self.panel, id=wx.ID_ANY, label='清', size=(30, 28))
        self.rext_clear_btn.SetBackgroundColour(wx.Colour(250,250,250))
        self.rext_clear_btn.Bind(wx.EVT_BUTTON,self.clearRoot_allext)
        self.rext_clear_btn.SetToolTip('清除文件类型排除')
        self.rbtn_sizer.Add(self.rext_clear_btn,0,wx.TOP,0)
        # 获取所有文件类型
        self.rext_getall_btn = wx.Button(self.panel, id=wx.ID_ANY, label='获', size=(30, 28))
        self.rext_getall_btn.SetBackgroundColour(wx.Colour(250, 110, 90))
        self.rext_getall_btn.Bind(wx.EVT_BUTTON, self.get_rallext)
        self.rext_getall_btn.SetToolTip('获取所有文件类型')
        self.rbtn_sizer.Add(self.rext_getall_btn, 0, wx.TOP, 0)
        # self.ext_getall_btn.Hide()
    # 读取远程设置
        self.rext_read_btn = wx.Button(self.panel, id=wx.ID_ANY, label='读', size=(30, 28))
        self.rext_read_btn.SetBackgroundColour(wx.Colour(130,250,190))
        self.rext_read_btn.Bind(wx.EVT_BUTTON,self.get_rextfilter_saver)
        self.rext_read_btn.SetToolTip('读取保存设置')
        self.rbtn_sizer.Add(self.rext_read_btn,0,wx.TOP,3)
        self.rext_read_btn.Hide()
    # 读取全部文件类型
        self.rext_readall_btn = wx.Button(self.panel, id=wx.ID_ANY, label='全', size=(30, 28))
        self.rext_readall_btn.SetBackgroundColour(wx.Colour(110,240,160))
        self.rext_readall_btn.Bind(wx.EVT_BUTTON,self.read_rallext)
        self.rext_readall_btn.SetToolTip('读取全部文件类型')
        self.rbtn_sizer.Add(self.rext_readall_btn,0,wx.TOP,3)
...
# 根目录文件类型排除文本
        ...
        if conf.has_option('set_DS', f'ROOTFILTER_{pp}'):
            rootfilter = conf.get('set_DS', f'ROOTFILTER_{pp}')
        self.root_filter = wx.TextCtrl(self.panel, value=rootfilter, style=wx.TE_PROCESS_ENTER|wx.TE_MULTILINE)
        self.root_filter.SetToolTip('排除文件后缀名，用;(分号)隔开(按回车保存)')
        self.root_filter.SetBackgroundColour(wx.Colour(242, 246, 240))
        self.root_filter.SetForegroundColour(wx.Colour(137, 146, 80))
        self.root_filter.SetFont(font)
...
# 虚拟表格：grid_out_v
        ...
        self.grid_out_v.SetDefaultCellBackgroundColour(wx.Colour(242, 250, 247))  #设置背景色
...
# 初始化时调用：
    ...
    def HelpFun(self,event):
        os.startfile('https://my.feishu.cn/wiki/Kq02wqBpKitI4Zkx2QgcsVKLnRV?from=from_copylink')

    def Rupdate(self,event):
        dlg = wx.MessageDialog(self, "确定要手动下载最新版本？", "[更新]", wx.YES_NO | wx.YES_DEFAULT | wx.ICON_INFORMATION)
...
# 设置数据库db
    ...
    def Move_list(self,event):  #移动文件
        self.TQ_MO(Ti=False)
    def TiQu_list(self,event):  #提取文件
        self.TQ_MO()

    def TQ_MO(self, Ti=True):
            ...
            if selected_rows:
                ...
                if result == wx.ID_YES:
                    ...
                    for L in selected_rows:
                        if self.grid_out.IsShown():
                            selected_cells = self.grid_out.GetCellValue(L, 3)
                        else:
                            selected_cells = self.grid_out_v.GetCellValue(L, 3)
                        if selected_cells:
                            if os.path.exists(selected_cells):
                                ...
                                if Ti:
                                    ...
                                    shutil.move(selected_cells, c_p)
                                    if self.grid_out.IsShown():
                                        self.grid_out.SetCellValue(L, 1, 'del')
                                        self.grid_out.SetCellTextColour(L, 3, wx.Colour(182, 180, 180))
                                    else:
                                        self.grid_out_v.SetCellValue(L, 1, 'del')
                                ccc += 1
                    if ccc > 0:
                        dlg = wx.MessageDialog(self, f"共处理 {ccc} 个文件", "文件提取", wx.ICON_QUESTION)
                        dlg.ShowModal()
                        dlg.Destroy()
            else:
                        ...
                        for file_name in files:
                            if self.StartStop:
                                ...
                                if self.search_fol_chk.GetValue():
                                    ...
                                    if self.ignoreFile(self.root_filter.GetValue(), ex): continue  # rootfilter类型排除
        # 查重筛选文件
...
#刷新查重菜单
    ...
    def Get_DifMenu(self,event,Ff):
        ...
        self.SelDif = True
        self.StartStop = True
        self.ZhuangTai = 'CC'   #用于判断当前列表获取的状态（CC 查重，SS 搜索）
            ...
            if kcunt > 0:
                ...
                for k in md5er:
                    if self.StartStop:
                        ...
                        self.ResetWinSize(row)
                        return
        self.text_label2.SetLabel('操作完成。')
...
#自定义排序函数
    ...
    PerCopyFiles = dict()
    def del_oldfile(self,event):   #删除已上传文件
        if self.list.GetCount()>0:
            dlg = wx.MessageDialog(self, "确定删除路径下方列表选中的已上传对象？", "[删除已上传]", wx.YES_NO | wx.ICON_QUESTION)
            result = dlg.ShowModal()
            dlg.Destroy()
            if result == wx.ID_YES:
                sels = self.list.GetSelections()
                if sels:
                    allsel = [self.list.GetString(i) for i in sels]
                    for p in allsel:
                        index = self.list.FindString(p)
                        try:
                            if '(只读)' in p:
                                stmp = f'{p}'
                                stmp=stmp.replace("(只读)", "", 1)
                                os.chmod(stmp, stat.S_IWRITE)
                                os.remove(stmp)
                            ...
                            if '(只读)' not in p:
                                self.list.SetString(index,f'(只读){p}')
            self.list.SetToolTip(f'剩余 {self.list.Count} 个文件已上传数据库')
                            ...
                            if selected_indices:
                                ...
                                if scount > 0:
                                    ...
                                    if len(self.PerCopyFiles)>0:
                                        ...
                                        self.cpy_btn.Hide()
                                        self.ResetWinSize(0)
                                        self.text_label2.SetLabel('未找到待拷贝文件')
...
#导入文件并更新数据库
    def click_cpybtn(self,event):   #开始复制
        if len(self.PerCopyFiles)>0:
            if self.grid_out.IsShown():
                selected_rows = self.grid_out.GetSelectedRows()
                rowc = self.grid_out.GetNumberRows()
            else:
                selected_rows = self.grid_out_v.GetSelectedRows()
            if selected_rows:
                ...
                if result == wx.ID_YES:
                    ...
                    if gnum == 0:
                        ...
                        self.ResetWinSize(0)
                        if self.grid_out.IsShown():
                            self.grid_out.DeleteRows(0, rowc)
                        else:
                            table = MyGridTable([])
                            self.grid_out_v.SetTable(table, True)
                    else:
                        ...
                        if self.grid_out.IsShown():
                            self.grid_out.DeleteRows(0, rowc)
                            row = 0
                            for i in self.PerCopyFiles.values():
                                col=0
                                self.grid_out.AppendRows()
                                for d in i[0]:
                                    self.grid_out.SetCellValue(row, col, d)
                                    ...
                                    col += 1
                                self.grid_out.SetCellTextColour(row, 1, wx.Colour(182, 180, 187))
                                self.grid_out.SetCellBackgroundColour(row, 6, wx.Colour(225, 250, 210))
                                self.grid_out.SetCellBackgroundColour(row, 0, wx.Colour(225, 250, 240))
                                self.grid_out.SetCellBackgroundColour(row, 2, wx.Colour(225, 225, 225))
                                row += 1
                            self.ResetWinSize(gnum)
                        else:
                            table = MyGridTable([])
                            self.grid_out_v.SetTable(table, True)
                            updata = []
    ...
    def on_char_hook(self, event):  #快捷键
        ...
        elif keycode == ord('I') and (modifiers & wx.MOD_CONTROL) and (modifiers & wx.MOD_ALT) and (modifiers & wx.MOD_SHIFT):
            if NET:
                ...
                if second_frame.ShowModal() == wx.ID_OK:  # 锁定主窗口
                    value = second_frame.get_input()
                    if value == 'zyhp':
                        verset=redis.StrictRedis(host=s_host, port=6379, db=0)
                        self.text_label.SetLabel('<正确>')
                        vv = int(verset.get('version'))
                        if vv < vvv:
                            verset.set('version', f'{vvv}')
                            program_name = os.path.basename(sys.argv[0])
                            dup_file='\\\\192.168.10.38\\自研产品部\\S1产品部\\Softs\\DUPCLICKER'
    ...
    #该组合键判断需要从多到少进行判断，避免出现bug
        elif keycode == ord('A') and (modifiers & wx.MOD_CONTROL) and (modifiers & wx.MOD_ALT) and (modifiers & wx.MOD_SHIFT):
        ...
        # if (modifiers & wx.MOD_CONTROL) or (modifiers & wx.MOD_ALT) or (modifiers & wx.MOD_SHIFT):
        if self.grid_out.IsShown():
            selected_rows = self.grid_out.GetSelectedRows()
        else:
            selected_rows = self.grid_out_v.GetSelectedRows()
        if selected_rows:
            try:
                ...
                elif keycode == ord('D') and modifiers == wx.MOD_ALT:
                        ...
                        if self.cpy_btn.IsShown():
                            ...
                            if selected_indices:
                                ...
                                delPC = list(self.PerCopyFiles.values())
                                    ...
                                    # 将编号顺延顶替
                                    # for i in range(len(self.PerCopyFiles), 0, -1):
                                    #     if i not in self.PerCopyFiles:
                                    #         self.PerCopyFiles[i] = self.PerCopyFiles.pop(i + 1)
                                try:
                                ...
                                if self.grid_out.IsShown():
                                    ...
                                    if gnum>0:
                                        if self.cpy_btn.IsShown():
                                            for i in range(gnum):  #重新排序待拷贝文件
                                                self.PerCopyFiles[i] = delPC[i]
                                            self.ResetWinSize(gnum)
                                else:
                                    for i in dels:
                                        self.grid_out_v.DeleteRows(i)
                                    gnum = self.grid_out_v.GetNumberRows()
                                    if gnum>0:
                                        if self.cpy_btn.IsShown():
                                            updata = []
                                            for i in range(gnum):
                                                self.PerCopyFiles[i] = delPC[i]
                                                updata.append(delPC[i][0])
                                            table = MyGridTable(updata)
                                            self.grid_out_v.SetTable(table, True)
                                            self.ResetWinSize(gnum)
                                if msg != '':
                ...
                elif keycode == ord('S') and (modifiers & wx.MOD_ALT):
                    ccc = 0
                    self.progress_bar.SetRange(len(selected_rows))
                    if self.grid_out.IsShown():
                        for L in selected_rows:
                ...
                elif keycode == ord('C') and (modifiers & wx.MOD_CONTROL) and (modifiers & wx.MOD_ALT):
                    self.Grid_sel_call(self)
            except:
                pass
    ...
    def get_rallext(self,event):
        self.progress_bar.SetValue(0)
        pp = self.getNoSlashPath(self.path_combox.GetValue())
        path = self.path_combox.GetValue()
        doit = False
        if path.startswith('\\'):
            dlg = wx.MessageDialog(self, "确定要重新获取全部文件类型吗？", "[获取文件类型]", wx.YES_NO|wx.YES_DEFAULT|wx.ICON_INFORMATION)
            result = dlg.ShowModal()
            dlg.Destroy()
            if result == wx.ID_YES:
                doit = True
        ...
        if doit:
            ...
            for root, dirs, files in os.walk(path):
                ...
                for file_name in files:
                    ...
                    ext = str(file_extension[1:]).lower()
                    if ext not in extensions:
                        extensions+=(ext+'; ')
                        extc+=1
                    CCC += 1
                    self.text_label.SetLabel(str(CCC))
                    if CCC % 40 == 0:
                        wx.Yield()
            if extc>0:
                self.text_label2.SetLabel(f'共找到 {extc} 个类型')
                self.rext_readall_btn.Show()
                self.root_filter.SetValue(extensions)
                self.panel.Layout()
                conf.set('set_DS', f'ROOTFILTERALL_{pp}', extensions)
                conf.write(open(setPath, 'w+', encoding="utf-8"))

    def read_rallext(self,event):
        pp = self.getNoSlashPath(self.path_combox.GetValue())
        if conf.has_option('set_DS', f'ROOTFILTERALL_{pp}'):
    ...
    def clear_allext(self,event):
        ...
        # conf.set('set_DS', f'FOLFILTER_{pp}_{username}', '')
        ...
        self.panel.Layout()
        if NET:
            self.setr.hset('SETTINGS', f'EXTFILTER_{pp}_{username}', '')
    ...
    def get_rextfilter_saver(self, event):
        pp = self.getNoSlashPath(self.path_combox.GetValue())
        if conf.has_option('set_DS', f'ROOTFILTER_{pp}'):
            self.root_filter.SetValue(conf.get('set_DS', f'ROOTFILTER_{pp}'))
            self.FindTxt()

    def on_rextfilter_saver(self, event):
        pp = self.getNoSlashPath(self.path_combox.GetValue())
        rot = self.root_filter.GetValue()
        if rot != '':
            self.rext_read_btn.Show()
            conf.set('set_DS', f'ROOTFILTER_{pp}', rot)
            conf.write(open(setPath, 'w+', encoding="utf-8"))
            self.FindTxt()
            self.panel.Layout()
    ...
    def check_extfilter(self,event=None):   #让 event 可选，在调用时直接执行 self.check_extfilter()
        pp = self.getNoSlashPath(self.path_combox.GetValue())
        if self.search_fol_chk.GetValue():
            checkbox = 'True'
            self.list.Hide()
            ...
            if extstring != '':
                self.ext_readall_btn.Show()
            self.v_sizer.Detach(self.rext_sizer)
            ...
            self.inputData_btn.Enable(False)
            self.list.Show()
            self.root_filter.Show()
            # self.h_sizer_list.Detach(self.ext_sizer)
            self.v_sizer.Add(self.rext_sizer, proportion=3, flag=wx.EXPAND|wx.LEFT, border=1)
            self.ext_filter.Hide()
            self.fol_filter.Hide()
            self.ext_getall_btn.Hide()
            self.ext_clear_btn.Hide()
            self.rext_getall_btn.Show()
            self.rext_clear_btn.Show()
            if conf.has_option('set_DS', f'ROOTFILTERALL_{pp}'):
                self.root_filter.SetValue(conf.get('set_DS', f'ROOTFILTERALL_{pp}'))
                self.rext_readall_btn.Show()
            if conf.has_option('set_DS', f'ROOTFILTER_{pp}'):
        ...
        self.on_comb_return(self)
        self.panel.Layout()
        conf.set('set_DS', 'searchFol_chk', checkbox)
                ...
                if result == wx.ID_YES:
                    ...
                    if os.path.exists(path):
                        ...
                        if len(oldfilep) > 0:
                            ...
                            self.panel.Layout()
                            self.list.Clear()
                            self.list.Set(oldfilep)
                            self.list.SetToolTip(f'共 {len(oldfilep)} 个文件已上传数据库')
                        else:
                            self.list.Clear()
                            self.list.SetToolTip(f'0 个文件已上传数据库')
                        if DEL:
                            self.r.close()
                        # 删除主路径旧存档
                            pp = hkey.split(' → ')[0]
    ...
    def on_comb_return(self, event):    #路径下来列表事件
        ...
        if os.path.exists(path):
            ...
            if self.search_fol_chk.GetValue():
                ...
                if conf.has_option('set_DS', f'ROOTFILTER_{pp}'):
                    rextstring = conf.get('set_DS', f'ROOTFILTER_{pp}')
                    self.root_filter.SetValue(rextstring)
                    if rextstring != '':
                        self.rext_read_btn.Show()
                else:
                    self.root_filter.SetValue('')
        ...
        if selected_indices:
            ...
            if self.SoftSearch:
                ...
                if not hasattr(self, 'data'):
                    self.data = []
                # else:
                #     dlg = wx.MessageDialog(self, "检查数据库是否链接正确。", "[链接错误]", wx.YES_NO | wx.YES_DEFAULT | wx.ICON_INFORMATION)
                #     result = dlg.ShowModal()
                #     dlg.Destroy()
